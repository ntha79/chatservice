...
class ContactsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Contacts entity")
        ...
        .repeat(2) {
            ...
            .get("/chatservice/api/contacts")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new contacts")
            .post("/chatservice/api/contacts")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "seqId":"SAMPLE_TEXT", "ownerId":null, "ownerLogin":"SAMPLE_TEXT", "contactLists":"SAMPLE_TEXT", "contactCount":"0", "createdBy":"SAMPLE_TEXT", "createdDate":"2020-01-01T00:00:00.000Z", "createdUnixTime":null, "lastModifiedBy":"SAMPLE_TEXT", "lastModifiedDate":"2020-01-01T00:00:00.000Z", "lastModifiedUnixTime":null, "reportDay":"0"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_contacts_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created contacts")
                .get("/chatservice${new_contacts_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created contacts")
            .delete("/chatservice${new_contacts_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
